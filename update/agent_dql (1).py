...
class Agent():
    ...
    def _select_action(self, state, learning=True):
      if learning and np.random.rand() < self.exploration:
          action_probs = torch.ones(self.num_actions, device=self.device) / self.num_actions
          ...
          if state is None or state.shape[0] != self.input_dim:
              ...
              try:
                  if response_vector.sum() > 0:
                      response_idx = torch.argmax(response_vector).item()
              except Exception as e:
                  if self.verbose:
                      print(f"Error in calculating response_idx: {e}, response_vector: {response_vector}")
                  response_idx = 0

          # Gán xác suất dựa trên trạng thái
          if response_idx == 0:
              if self.last_response_idx in [3, 4]:
                  action_probs[30:40] *= 4.0
                  action_probs[40:70] *= 2.5
                  action_probs[12:30] *= 0.5
              else:
                  action_probs[0:12] *= 2.5
                  action_probs[12:30] *= 1.2
          elif response_idx in [1, 2]:
              with torch.no_grad():
              ...
              action_probs[30:40] *= 4.0
              action_probs[40:70] *= 2.5
              action_probs[12:30] *= 0.8
              if self.last_response_idx == 3:
                  action_probs[30:40] *= 5.0
                  action_probs[12:30] *= 0.3
          elif response_idx == 4:
              action_probs[30:40] *= 4.0
              action_probs[40:70] *= 2.5
              if self.column_count == 3:
                  action_probs[18:21] *= 3.0
                  action_probs[12:18] *= 0.2
                  action_probs[21:30] *= 0.2
              else:
                  action_probs[18:21] *= 2.5
                  action_probs[12:30] *= 0.8
              if self.last_response_idx == 4:
                  action_probs[30:40] *= 5.0
                  action_probs[12:30] *= 0.3
          elif response_idx == 5:
              action_probs[30:70] *= 2.5
              for action in self.waf_blocked_actions:
                  action_probs[action] *= 0.1

          # Giảm xác suất hành động thất bại
          if self.action_taken and self.last_reward < 0:
              last_action = self.action_taken[-1]
              action_probs[last_action] *= 0.5

          # Kết hợp với Q-value
          if response_idx not in [1, 2]:
              with torch.no_grad():
                  q_values = self.model(state.unsqueeze(0))
                  q_probs = torch.softmax(q_values, dim=1).squeeze()
                  action_probs = 0.7 * action_probs + 0.3 * q_probs
    ...
    def step(self, env):
        ...
        if state_resp == 4 and self.action_taken and self.action_taken[-1] in range(12, 30):
            self.column_count = 3
        if state_resp == 5:
            self.waf_blocked_actions.append(self.action_taken[-1])
